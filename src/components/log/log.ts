...
export const logStore = new Vuex.Store({
  ...
  mutations: {
    [PUSH_USER_DEBUG_LOG] (state, payload: Record<string, any>) {
      state.props.push(payload)
    }
  }
})

export function USER_DEBUG_LOG (userMessage: string, debugMessage?: Record<string, unknown>, type?: USER_DEBUG_LOG_TYPE) {
