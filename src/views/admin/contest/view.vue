...
a-card(:title="contestDetail.title", hoverable, :loading="loading")
  ...
  a-space(direction="vertical", style="width: 100%;")
    ...
    a-card(:title="'题目列表' + (contestDetail.problems ? (contestDetail.problems.length === 0 ? '为空' : '') : '为空')")
...
import { ContestDetailInterface, apiContestDetail } from '@/map/api';
import { getNKPCUrl, getProblemUrl } from '@/map/objFormatUrl';
...
export default class extends Vue.with(Props) {
  ...
  contestDetail = {} as ContestDetailInterface;
