...
div.cc-container
  a-spin(:spinning="spinning")
    ...
    a-form(v-else, :labelCol="{ span: 9 }", :wrapperCol="{ span: 9 }", style="max-width: 800px; min-width: 600px;")
      ...
      a-form-item(v-show="needCaptcha", label="验证码")
        a-input(v-model:value="signInPackage.captcha")
          template(#suffix)
            a-image(@error="captchaLoadError", :src="captchaSrc", @load="captchaLoad", height="30px", width="90px")
          template(#addonAfter)
            ReloadOutlined(style="cursor: pointer;", @click="captchaReset")
